...
def configured(mock_cpu_files_empty, monkeypatch):
    ...
    def no_discovery():
        raise AssertionError("Explicit allocation must not rediscover isolated CPUs")

    monkeypatch.setattr("epa_orchestrator.cpu_pinning.get_isolated_cpus", no_discovery)
    return provider
...
def api(provider, action="list_allocations", service="owner", **params):
    ...
    request = {"action": action, "service_name": service, **params}
...
def test_protected_ordinary_pool_conflict_restart_and_release(configured, action, params, field):
    ...
    assert listing["supported_cpu_features"] == ["non-preemptive-allocations"]
...
def test_failed_online_check_does_not_upgrade_or_reclaim(
    ...
    monkeypatch.setattr(configured, "check_online", unavailable)
...
def test_pool_conflict_validation_uses_locked_fresh_claims(configured, monkeypatch):
    ...
    def claim_before_lock(operation):
        AllocationsDB().allocate_cores("other", "7", PreemptionPolicy.NON_PREEMPTIVE)
